...
class TravelingSalesmenProblem:
    def __init__(self,
        ...
        self.edge_weight_type = (edge_weight_type or "EUC_2D").upper()
        ...
        if self.node_coord_dict is not None:
            ...
            if distance_mat is not None and np.size(distance_mat) > 0:
                self.distance_mat = np.array(distance_mat)
            else:
                self.distance_mat = self.calculate_euc_distance_matrix(self.node_coord_dict)

        elif distance_mat is not None and np.size(distance_mat) > 0:
    ...
    def euclidean_distance(self, point1, point2):
        """
        Return the TSPLIB-compliant distance between two points after applying
        the rounding rule dictated by EDGE_WEIGHT_TYPE.
        """
        ...
        # Default to EUC_2D rounding when type is unspecified or unsupported.
